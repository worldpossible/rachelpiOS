...
def install_kalite():
	...
	sudo("cp -r /home/rachel/.kalite /home/root/.kalite") or die("Unable to copy rachel data.")
	sudo("rm -fr /home/rachel/.kalite") or die("Removing rachel data from rachel user.")
...
def install_stemmer():
	stem_url = "https://pecl.php.net/get/stem-1.5.1.tgz"
	patch_url = "https://bugs.php.net/patch-display.php?bug_id=71091&patch=feature-support-php5-and-php7&revision=1457366472&download=1"
	sudo("wget '" + stem_url + "' -O /tmp/stem-1.5.1.tgz") or die("Can't download stem")
	sudo("wget '" + patch_url + "' -O /tmp/stem-php7.patch") or die("Can't download stem patch")
	sudo("mkdir /tmp/stem-1.5.1") or die("Can't make stem tmpdir")
	sudo("tar zxpf /tmp/stem-1.5.1.tgz --directory /tmp") or die("Can't extract stem")
	sudo("patch /tmp/stem-1.5.1/stem.c /tmp/stem-php7.patch") or die("Cant patch stem")
	sudo("sh -c 'cd /tmp/stem-1.5.1 && phpize'") or die("Can't PHPize stem")
...
sudo("apt update -y") or die("Unable to update.")
sudo("apt install -y git") or die("Unable to install Git.")
...
sudo("apt update -y") or die("Unable to update.")
sudo("apt dist-upgrade -y") or die("Unable to upgrade Raspbian.")
...
sudo("apt -y install apache2 libxml2-dev \
